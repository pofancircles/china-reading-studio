...
type Package = MaterialSource & {
  ...
  quality: {
    ...
    details?: { has_violations: boolean; has_overlength_sentences: boolean; has_forbidden_grammar?: boolean; status: "ok" | "needs_review" | "unavailable"; source_limited?: boolean; short_text_target?: [number, number] | null; source_difficulty_limited?: boolean; out_of_level_count?: number; level_profile?: LevelProfile };
  };
...
function GenerationWarning({ meta, onRetry, retrying, pendingComponent }: { meta: Package["meta"]; onRetry: () => void; retrying: boolean; pendingComponent?: RetryableComponent | null }) {
  ...
  const rewriteFailed = failedNames.includes("rewrite") || blockedNames.includes("rewrite");
  ...
  return <div role={pendingComponent ? "status" : "alert"} aria-live="polite" aria-busy={Boolean(pendingComponent) || retrying} className="mb-4 flex flex-col gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 md:flex-row md:items-start md:justify-between"><div><p><strong>{headline}</strong>{visibleFallbackCode && <> {fallbackMessage(visibleFallbackCode)}</>}</p>{failedNames.length > 0 && <p className="mt-1 text-xs leading-5"><strong>需要处理：</strong>{failedNames.map(componentLabel).join("、")}未通过硬校验，请在对应标签中单独重试；其他结果不会丢失。</p>}{blockedNames.length > 0 && <p className="mt-1 text-xs leading-5"><strong>尚未生成：</strong>{blockedNames.map(componentLabel).join("、")}</p>}{detailLines.length > 0 && <p className="mt-1 text-xs leading-5"><strong>阻塞原因：</strong>{detailLines.join("；")}</p>}{warningLines.length > 0 && <details className="mt-2 text-xs leading-5"><summary className="cursor-pointer font-semibold">查看 {warningLines.length} 条非阻塞等级建议</summary><p className="mt-1 text-slate-600">{warningLines.join("；")}</p></details>}</div>{pendingComponent ? <span className="inline-flex shrink-0 items-center gap-1.5 text-xs font-semibold"><LoaderCircle size={14} className="animate-spin" />正在重试当前组件…</span> : rewriteFailed ? <RetryButton onClick={onRetry} retrying={retrying} wholePackage /> : null}</div>;
}
